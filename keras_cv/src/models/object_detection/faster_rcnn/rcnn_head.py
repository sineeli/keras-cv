        ...
        **kwargs,
        ...
        for idx, conv_dim in enumerate(conv_dims):
            conv = keras.layers.Conv2D(
                ...
                name=f"rcnn_head_conv_{idx}",
            )
            self.convs.append(conv)
            self.norms.append(
                keras.layers.BatchNormalization(name=f"rcnn_head_b_{idx}")
            )

        self.fcs = []
        for idx, fc_dim in enumerate(fc_dims):
            layer = keras.layers.Dense(
                units=fc_dim, activation="relu", name=f"rcnn_head_fc_{idx}"
            )
        ...
        self.box_pred = keras.layers.Dense(
            ...
            name="rcnn_head_bbox_pred",
        )
        self.cls_score = keras.layers.Dense(
            units=num_classes + 1,
            kernel_initializer=keras.initializers.RandomNormal(stddev=0.01),
            name="rcnn_head_cls_score",
        )
